...
local function Time1()
	...
	time1 = string.format("%d", time_pos)
...
local function Time2()
  ...
  local video_id = url:match("v=([^&]+)")
	...
	local time2 = string.format("%d", time_pos)
	-- timeCopy = string.format("[%s](%s#t=%s,%s)", tt, url, time1, time2)
  timeCopy = string.format([[<iframe src="https://www.youtube.com/embed/%s?start=%s&end=%s" height="360" width="95%%" seamless="seamless" frameborder="0" allowfullscreen></iframe>]], video_id, time1, time2)
